...
$(document).ready(function () {
  ...
  $(document).on('click', '.next-step', function (e) {
    e.preventDefault();
    var errorMessages = '';
    $('.step-form__item.active :input[required]').each(function () {
      var $input = $(this);

      if (!$input.val().trim()) {
        var fieldName = $input.attr('placeholder') || $input.attr('name');
        errorMessages += 'Будь ласка, заповніть поле "' + fieldName + '".<br>';
        $input.addClass('error'); // Додати клас "error"
      } else {
        $input.removeClass('error'); // Видалити клас "error", якщо поле заповнене
      }
    });
  ...
  $('.check_st').on('change', function () {
    ...
    if (th_.is(':checked')) {
      ...
      if (th_.closest('.checked-item').find('.hide-drop input').val() > 0) {} else {
        th_.closest('.checked-item').find('.hide-drop input').attr('required', 'required');
      }
    } else {
      th_.closest('.checked-item').find('.hide-drop').slideUp();
      th_.closest('.checked-item').find('.hide-drop input').attr('required', '').val('');
    }
  });
